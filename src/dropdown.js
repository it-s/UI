angular.module('com.likalo.ui')
  .directive('uiPicker', [
    ...
    '$document',
    '$uiTools',
    function ($window, $document, $uiTools) {
      return {
        restrict: 'EA',
        require: 'ngModel',
        replace: true,
        ...
        template: '<div class="UI-picker" ng-class="{active: expanded, bottom: !top, top: top}"><a class="pure-menu-link UI-picker--label layout--flex-row" ng-click="toggle($event)"><img class="UI-picker--label-image layout--flex-cell" ng-src="{{selected.src}}" ng-if="selected.src"/><ui-icon class="UI-picker--label-icon layout--flex-cell" type="{{selected.icon}}" ng-if="selected.icon"></ui-icon><span class="UI-picker--label-title layout--flex-grow" ng-bind="selected?selected.title:placeholder" ng-if="selected.title || !selected&&placeholder"></span><ui-icon class="UI-picker--carret layout--flex-cell" type="menu-down"></ui-icon></a><nav class="UI-picker--list"><ul class="pure-menu-list"><li class="pure-menu-item" ng-class="{active: item === selected}" ng-click="select(item)" ng-repeat="item in options"><span class="pure-menu-link"><img class="UI-picker--list-image" ng-src="{{item.src}}" ng-if="item.src"/><ui-icon class="UI-picker--list-icon layout--menu-icon" type="{{item.icon}}" ng-if="item.icon"></ui-icon><span class="UI-picker--list-label" ng-if="item.title">{{item.title}}</span></span></li></ul></nav></div>',
        link: function ($scope, $el, $attrs, ngModel) {

          var el = $uiTools.elementUnwrap($el),
            label = $el.children()[0],
            list = $el.children()[1];
